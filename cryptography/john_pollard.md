...
After downloading the certificate, the file type is seen using ```file cert```:
```
cert: PEM certificate
```

On research it is found that an X.509 certificate is used to secure communication and transactions between two parties. It includes information about the certificate holder.

Thus to break the RSA certificate we use the following command:
```
openssl x509 -in cert -text -noout
```

This is because no output is needed.

The output is the certificate details:
```
Certificate:
    Data:
        Version: 1 (0x0)
...
Based on hints of finding p and q it could mean factoring the modulus. We use this online tool:
```
https://www.alpertron.com.ar/ECM.HTM
```

The output of factoring the modulus 4966306421059967 is:
```
67 867967 × 73 176001
```

The first flag tried is:
```
picoCTF{67867967,73176001}
```


But that is incorrect, so values are swapped based on hints to find the correct flag:
```
